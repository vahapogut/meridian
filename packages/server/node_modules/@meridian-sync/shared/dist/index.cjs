...
var RuleEvaluator = class {
   ...
   */
  async check(collection, operation, auth, doc = { existing: null, incoming: null }) {
   ...
   */
  async filterRead(collection, auth, docs) {
    const allowed = [];
   ...
   */
  async canWrite(collection, auth, existing, incoming) {
...
function createTransport(config) {
  switch (config.type) {
    ...
    case "kafka":
      throw new Error(
        `Transport "${config.type}" is not yet implemented. WebSocket is the default. Others are on the roadmap.`
      );
    default:
...
async function decryptValue(key, packed) {
  const subtle = getSubtle();
...
async function encryptFields(doc, key, fields) {
  const result = { ...doc };
...
async function decryptFields(doc, key, fields) {
  const result = { ...doc };
  for (const field of fields) {
    const value = doc[field];
...
async function encryptOperation(key, op) {
